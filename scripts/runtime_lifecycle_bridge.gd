...
func set_simulation_paused(
	...
	var clock := (
		clock_variant as SimulationClock
	)

	if paused:
		_previous_process_enabled = (
			game.is_processing()
		)
		_previous_physics_process_enabled = (
			game.is_physics_processing()
		)
		_previous_unhandled_input_enabled = (
			game.is_processing_unhandled_input()
		)
		...
		# Main's own process toggles do not implicitly disable
		# child Node processing. Freeze presentation paths that
		# mirror state into the simulation before saving.
		if game.has_method(
			"_set_runtime_paused"
		):
			game.call(
		...
		game.set_physics_process(
			false
		)
		game.set_process_unhandled_input(
			false
		)
		return
